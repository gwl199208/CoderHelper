...
namespace Coder.WebUI.Dto.Tool
{
    /// <summary>
    /// 做类型转换的工具类
    /// </summary>
    ...
    {
        public static UserModel RegisterConvert(RegisterInput input)
        {
            UserModel register = new UserModel() { username = input.username , password = input.password , email = input.mail , 
            sex = input.sex , school = input.school , nickname = input.nickname};
            
            register.solvedNumber = register.submitNumber = 0;
            return register;
        }
